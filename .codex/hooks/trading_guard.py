#!/usr/bin/env python3
"""Codex safety guard for kis-trader.

This hook is intentionally conservative. It blocks the highest-risk operations
that should never be performed by an agent in this repository.
"""
...
MAIN_PY_BOUNDARY_REASON = (
    'app/main.py is a thin entrypoint (AGENTS.md): do not add a new '
...
def main() -> None:
    ...
    if PROTECTED_PATH_RE.search(text) or LEGACY_CACHE_VARIANT_RE.search(text):
        _deny("Protected credential/token files must not be read, printed, or edited by Codex.")

    if tool_name in {"Bash", "bash"}:
        command = _command_text(tool_input)
        if APP_MAIN_RE.search(command):
            _deny("Direct app.main execution is blocked. Do not start the trading bot from Codex.")
        if RUN_SESSION_RE.search(command):
            _deny("Direct run_session.sh execution is blocked. Do not start trading sessions from Codex.")
